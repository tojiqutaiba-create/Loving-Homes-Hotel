...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (bookingForm) {
        bookingForm.addEventListener("submit", function(event) {
            ...
            let petGender = document.querySelector('input[name="petGender"]:checked').value;
    ...
    if (contactForm) {
        contactForm.addEventListener("submit", function(event) {
            event.preventDefault();
            alert("تم استلام رسالتك بنجاح! شكراً لتواصلك معنا، سيتواصل معك فريقنا خلال 24 ساعة.");
            contactForm.reset();
        });
    }
